...
func (c *Connector) GetAadLogin(ctx context.Context, name string) (*model.AadLogin, error) {
  ...
  err := c.QueryRowContext(ctx,
    "SELECT principal_id, name, CONVERT(VARCHAR(85), [sid], 1), default_database_name, default_language_name FROM [master].[sys].[server_principals] WHERE [type] NOT IN ('G', 'R') and [name] = @name",
