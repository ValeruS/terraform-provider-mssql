# mssql_server_role_member (Data Source)

The `mssql_server_role_member` data source reads the list of members of a server-level role.
## Example Usage

```hcl
data "mssql_server_role_member" "example" {
  server {
    host = "localhost"
    login {
      username = "sa"
...
output "members" {
  value = data.mssql_server_role_member.example.members
}
```
...
```hcl
data "mssql_server_role_member" "state_reader" {
  server {
...
* `members` - Set of login names that are members of the role.
