# mssql_entraid_login (Data Source)

The `mssql_entraid_login` obtain information about an existing EntraID (formerly Azure AD) login in SQL Server.

## Example Usage

```hcl
data "mssql_entraid_login" "example" {
  server {
...
* `login_name` - (Required) The name of the EntraID login to look up.
